...
MODES = {"default": (GRY, "manual"), "plan": (CYN, "plan"),
         "acceptEdits": (MAG, "accept edits"), "auto": (YEL, "auto"),
...
def ledger_update(sid,cost,project,name,root=None,pid=None):
    ...
    if cost is not None:
        row={**prev,"updated":stamp,"state":"live",
    ...
    return {"all":sum(r.get("cost",0) for r in rows),"n":len(real),
...
def rl_log(node5,node7):
    ...
    """
    try:
        cur={"5h":dig(node5,"used_percentage"),"5h_reset":dig(node5,"resets_at"),
             "7d":dig(node7,"used_percentage"),"7d_reset":dig(node7,"resets_at")}
        last=None
        if os.path.exists(RLHIST):
            ...
            if tailed:
                try: last=json.loads(tailed[-1])
                except Exception: last=None
        if last and all(last.get(k)==v for k,v in cur.items()): return
        cur["at"]=ledger.iso()
        with open(RLHIST,"a") as fh: fh.write(json.dumps(cur)+"\n")
    except Exception:
        pass
...
def limit_seg(label,node,window_h):
    ...
    if resets:
        ...
        if pct<100:
            ...
            burn=pct/elapsed; proj=burn*window_h
...
def main():
    ...
    try:
        tmp=PAYLOAD+".tmp"
        with open(tmp,"w") as fh: fh.write(raw)
        os.replace(tmp,PAYLOAD)
    except Exception: pass
    try: d=json.loads(raw)
    except Exception: print(f"{D}claude{R}"); return
    rows={}
    ...
    p1.append(f"{GRN}think{R}" if dig(d,"thinking","enabled",default=False)
              else f"{YEL}no-think{R}")
    ...
    if style and style!="default": p1.append(f"{CYN}{style}{R}")
    if t.get("tier") and t["tier"]!="standard": p1.append(f"{YEL}{t['tier']}{R}")
    ver=dig(d,"version")
    if ver: p1.append(f"{D}v{ver}{R}")
    ...
    pj.append(f"{BLU}{B}{proj}{R}" if here==proj
              else f"{BLU}{B}{proj}{R}{D}/{R}{BLU}{here}{R}")
    extra=dig(d,"workspace","added_dirs") or []
    if extra: pj.append(f"{D}+{len(extra)} dir{'s' if len(extra)!=1 else ''}{R}")
    ...
    if not gs:
        ...
        try:
            cands=[e.path for e in os.scandir(cwd)
                   if e.is_dir() and os.path.isdir(os.path.join(e.path,".git"))]
        except Exception:
            cands=[]
        if len(cands)==1:
            gs=pr.probe(f"git:{cands[0]}",3,lambda c=cands[0]: pr.git_state(c))
            if gs: cwd=cands[0]; nested=True
    if gs:
        br=gs["branch"]
        g=(f"{D}↳{os.path.basename(cwd)} {R}" if nested else "")+f"{RED if br in ('main','master') else GRN}{br}{R}"
        if gs.get("dirty"): g+=f"{YEL}*{R}"
        if gs.get("worktree"): g+=f"{CYN}⑂wt{R}"
        if gs.get("upstream"):
            if gs.get("ahead"): g+=f"{D}↑{R}{GRN}{gs['ahead']}{R}"
            if gs.get("behind"): g+=f"{D}↓{R}{RED}{gs['behind']}{R}"
            if not gs.get("ahead") and not gs.get("behind"): g+=f"{D}≡{R}"
        else:
            g+=f"{D}(no upstream){R}"
        if gs.get("stash"): g+=f" {D}stash{R}{YEL}{gs['stash']}{R}"
        pj.append(g)
    else:
        pj.append(f"{D}no git{R}")
    nm=dig(d,"session_name")
    if nm: pj.append(f"{D}“{nm}”{R}")
    ...
    live=sum(int(cur.get(k) or 0) for k in
    ...
    if size:
        ...
        if upct>=80: c+=f" {RED}{B}⚠{R}"
        segs.append(c)
    sess=[f"{D}session{R} {CYN}{tok(served)}{R}{D} in{R}",
          f"{CYN}{tok(t['out'])}{R}{D} out{R}"]
    if t["think"]: sess.append(f"{MAG}{tok(t['think'])}{R}{D} thinking{R}")
    sess.append(f"{t['turns']}{D} api turns{R}")
    if t["turns"]:
        sess.append(f"{D}avg{R} {tok(served//t['turns'])}{D}/{R}{tok(t['out']//t['turns'])}"
                    f"{D} per turn{R}")
    segs.append(f" {D}·{R} ".join(sess))
    if segs: rows["CONTEXT"]=row("CONTEXT",segs)
    ...
    if fh_:
        ...
        if over_active: seg+=f" {RED}{B}OVERAGE{R}"
        lim.append(seg)
    if dig(rl,"seven_day"): lim.append(limit_seg("7d",rl["seven_day"],168))
    if lim: rows["USAGE"]=row("USAGE",lim)
    ...
    if served:
        ...
        ttl=CACHE_TTL_S; win="1h"; obs=False
        if t.get("last_bucket"):
            win=t["last_bucket"]; ttl=3600 if win=="1h" else 300; obs=True
        age=None
        if t.get("last_ts"):
            ep=iso_epoch(t["last_ts"])
            if ep: age=time.time()-ep
        ...
        if age is not None and age>=0:
            if age>=ttl:
                ...
                w+=f"{D} ▸ expires {R}{time.strftime('%H:%M',time.localtime(time.time()-age+ttl))}"
        r3.append(w)
    ...
    tools=t["tools"]; ntool=sum(tools.values())
    ...
    if t["synth"]: r5.append(f"{RED}{t['synth']}{R}{D} api errors{R}")
    r5.append(f"{GRN}{len(t['f_edit'])}{R}{D} files edited{R} {D}·{R} "
              f"{len(t['f_read'])}{D} read{R}")
    if t["side"]: r5.append(f"{MAG}{t['side']}{R}{D} subagent msgs{R}")
    else: r5.append(f"{D}0 subagents{R}")
    cc=RED if t["compact"] else D
    r5.append(f"{cc}{t['compact']}{R}{D} compactions{R}")
    if t["cmds"]:
        r5.append(f"{D}cmds{R} "+" ".join(f"{D}{k}{R}{CYN}{v}{R}" for k,v in
                                          sorted(t["cmds"].items(),key=lambda kv:-kv[1])[:3]))
    ...
    r6=[]
    if t["durs"]:
        s=sorted(t["durs"]); med=s[len(s)//2]/1000; last=t["durs"][-1]/1000
        mx=max(t["durs"])/1000
        r6.append(f"{D}turn{R} {dur(last)}{D} last · {dur(med)} median · {dur(mx)} max"
                  f" ({len(t['durs'])} timed){R}")
    r6.append(f"{D}wall{R} {dur(wall)} {D}· api{R} {dur(api)}"
              +(f" {D}({api/wall*100:.0f}% busy){R}" if wall else ""))
    if t["hook_runs"]:
        hs=sorted(t["hook_ms"]) or [0]
        he=RED if t["hook_errs"] else D
        r6.append(f"{D}hooks{R} {t['hook_runs']}{D} runs · {hs[len(hs)//2]}ms median · {R}"
    ...
    if dsk.get("pct") is not None:
        r7.append(f"{D}disk{R}{bar(dsk['pct'],8,80,92)} {grade(dsk['pct'],80,92)}"
                  f"{dsk['pct']:.0f}%{R}{D} · {gb(dsk.get('free'))} free{R}")
    if r7: rows["SYSTEM"]=row("SYSTEM",r7)

    # ---------- row 8: money ----------
    usd=dig(cost,"total_cost_usd")
    agg=ledger_update(d.get("session_id","?"),usd,proj,dig(d,"session_name"),

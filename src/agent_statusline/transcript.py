...
def _blank():
    return {"in":0,"cw":0,"cr":0,"out":0,"turns":0,"last_ts":None,"think":0,
            "b1h":0,"b5m":0,"tools":{},"errors":0,"synth":0,"f_edit":[],"f_read":[],
            "side":0,"compact":0,"durs":[],"hook_runs":0,"hook_ms":[],"hook_errs":0,
            "cmds":{},"perm":None,"tier":None,"last_bucket":None}

def _absorb(tot,e):
    t=e.get("type")
    if e.get("isSidechain"): tot["side"]+=1
    if e.get("permissionMode"): tot["perm"]=e["permissionMode"]
    ...
    if t=="system":
        ...
        if st=="turn_duration" and e.get("durationMs"):
            tot["durs"].append(int(e["durationMs"])); tot["durs"]=tot["durs"][-60:]
        ...
        for b in body:
            ...
            if b.get("type")=="tool_use":
                ...
                if fp:
                    ...
                    if fp not in tot[key] and len(tot[key])<400: tot[key].append(fp)
            elif b.get("type")=="tool_result" and b.get("is_error"):
                tot["errors"]+=1

    if t!="assistant": return
    if m.get("model")=="<synthetic>": tot["synth"]+=1
    u=m.get("usage") or {}
    if not u: return
    ...
    if h1 or m5: tot["last_bucket"]="1h" if h1>=m5 else "5m"
    if u.get("service_tier"): tot["tier"]=u["service_tier"]
    if e.get("timestamp"): tot["last_ts"]=e["timestamp"]

...
| TEST-001 | Medium | The suite had no formatter, type checker, coverage measurement, or commit-time gate, and moving tool versions were installed directly from package indexes for hosted CI. A green pytest run was not sufficient evidence: the first broad repository review found destructive installer behavior (INSTALL-001 through INSTALL-026) despite the suite passing throughout. | 0.2.2 | Resolved by [ADR 0023](adrs/0023-use-a-locked-local-quality-gate.md): a committed `uv.lock`, and a pre-commit gate running Ruff, Black, and mypy from the locked environment, plus subprocess-aware coverage measurement at release review |

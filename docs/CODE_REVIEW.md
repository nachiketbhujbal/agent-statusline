...
| CI-GATE-002 | High | The aggregate policy accepted a false extension of the job's `always()` condition, a shell override that disabled inherited fail-fast behavior, and quoted or whitespace-varied YAML spellings of protected keys, allowing required failures to be skipped or masked. | 0.2.13 | Resolved by requiring the exact job condition, making the script self-contained with `set -eu`, forbidding enforcement-step shell overrides, recognizing semantically equivalent protected-key spellings, and exercising failures without inherited shell flags |

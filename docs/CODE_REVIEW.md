...
| CI-GATE-002 | High | The aggregate policy accepted a false extension of the job's `always()` condition and an enforcement-step shell override that disabled inherited fail-fast behavior, allowing required failures to be skipped or masked. | 0.2.13 | Resolved by requiring the exact job condition, making the script self-contained with `set -eu`, forbidding enforcement-step shell overrides, and exercising failures without inherited shell flags |
